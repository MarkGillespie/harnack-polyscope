...
# from https://gist.github.com/w1th0utnam3/a0189dc8a2c067ccb56e1de8c317b190
def draw_loglog_slope(fig, ax, origin, width_inches, slope, inverted=False, color=None, polygon_kwargs=None, label=True, labelcolor=None, label_kwargs=None, zorder=None):
    ...
    left_center_disp = a_disp + 0.5 * (c_disp - a_disp)
    left_center = ax.transData.inverted().transform(left_center_disp)
    ...
    offset_ylabel = [-1. * label_kwargs["fontsize"], 0.0] if not inverted else [1. * label_kwargs["fontsize"], 0.0]
    ...
    ax.annotate(f"${slope}$", left_center, xytext=offset_ylabel, textcoords='offset points', ha=ha_ylabel, va="center", zorder=zorder, **label_kwargs)
