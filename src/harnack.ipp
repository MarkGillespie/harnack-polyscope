...
ccl_device bool ray_nonplanar_polygon_intersect_T(
    ...
    for (uint iL = 0; iL < params.n_loops; iL++) {
        ...
        if (isCircular) {
            ...
            if (!printed_disk_normal) {
                std::cout << ("normal: (" + std::to_string(normal[0]) + ", " +
    ...
    auto disk_solid_angle = [&](uint iD, const T3& x, T3& grad) -> T {
        ...
        T sign         = copysign(1, L);
        T aL =
            std::abs(L); // use absolute value of length when evaluating solid
    ...
    auto total_solid_angle = [&](const T3& x, T3& grad) -> T {
        T omega = params.solid_angle_formula == 0
                      ? triangulated_solid_angle(x, grad)
