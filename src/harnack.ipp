...
ccl_device bool ray_spherical_harmonic_intersect_T(
    ...
    auto findEnclosingLevelsets = [](T f, T levelset, T frequency,
                                     T* lower_levelset, T* upper_levelset) {
        // 𝑓0 ← (𝑓(r𝑡)−𝜙)/(2𝜋𝜔)
        // 𝑓− ← (2𝜋𝜔)⌊𝑓0⌋ + 𝜙
        // 𝑓+ ← (2𝜋𝜔)⌈𝑓0⌉ + 𝜙
        T f0 = (f - levelset) / (static_cast<T>(2 * M_PI) * frequency);
        *lower_levelset =
            static_cast<T>(2 * M_PI) * frequency * std::floor(f0) + levelset;
        *upper_levelset =
            static_cast<T>(2 * M_PI) * frequency * std::ceil(f0) + levelset;
    };
    ...
    auto distanceToBothLevelsets = [&](T f, T lower_levelset, T upper_levelset,
                                       const T3& grad) -> T {
        T scaling = params.use_grad_termination ? fmax(len(grad), epsilon) : 1;
    ...
    if (params.frequency > 0) {
        ...
        findEnclosingLevelsets(evaluatePolynomial(pos), levelset, frequency,
                               &lower_levelset, &upper_levelset);
    }
    ...
    while (t < tMax) {
        ...
        T3 grad;
        if (params.use_grad_termination) grad = calculateGradient(pos);
        ...
        if (params.frequency < 0) { // just looking for a single level set
            ...
            dist = distanceToBothLevelsets(f, lower_levelset, upper_levelset,
                                           grad);
        }
...
template <typename T>
bool close_to_zero(T ang, T lo_bound, T hi_bound, T tol,
                   const std::array<T, 3>& grad, bool use_grad_termination) {
...
T triangulated_solid_angle(const packed_float3* pts, const packed_uint3* loops,
                           ...
                           const std::array<T, 3>& x, std::array<T, 3>* grad,
                           int grad_mode                = 0,
                           bool use_quick_triangulation = false) {
    T omega = 0;
...
T prequantum_solid_angle(const packed_float3* pts, const packed_uint3* loops,
    ...
    for (uint iL : polygonLoops) {
        ...
        omega +=
            prequantum_loop_solid_angle<T>(pts, iStart, N, x, grad, grad_mode);
    }
...
T loop_rotation_index(const std::vector<std::array<T, 3>>& xp,
                      const std::vector<T>& Lp, int verbosity = 0) {
    using T3 = std::array<T, 3>;
    size_t N = xp.size();

    // HACK: return 0 if any lines intersect, otherwise return 1
    auto det = [](const T3& a, const T3& b, const T3& c) {
        return dot(a, cross(b, c));
    };
    auto projectiveIntersection = [&](const T3& a0, const T3& a1, const T3& b0,
                                      const T3& b1) -> T {
        T c0 = +det(a0, b1, b0);
        T c1 = -det(a1, b1, b0);
    ...
    for (size_t iL = 0; iL < N; iL++) {
        ...
        for (size_t jL = 0; jL + 1 < iL; jL++) {
            ...
            if (ta >= 0 && ta <= 1 && tb >= 0 && tb <= 1) {
                ...
                if (dot(ia, ib) > 0) {
                    if (verbosity >= 1) {
                        std::cout << "found intersection at times " << ta
                                  << ", " << tb << " ( " << iL << " - " << jL
                                  << " )" << std::endl;
                        std::cout << "   a: " << a0 << " --- " << a1
                                  << std::endl;
    ...
    for (size_t iP = 0; iP < N; iP++) {
        ...
        if (verbosity >= 1) {
            std::cout << "    >> [rotation_index] ps[" << iP << "]: ("
                      << ps.back()[0] << ", " << ps.back()[1] << ")"
                      << std::endl;
        }
    }

    auto dot = [](const T2& v, const T2& w) {
        return v[0] * w[0] + v[1] * w[1];
    };
    auto cross = [](const T2& v, const T2& w) {
    ...
    for (uint i = 0; i < N; i++) {
        ...
        if (verbosity >= 1) {
            ...
            std::cout << "    >> [rotation_index] turn[" << i
                      << "]: " << turn_degrees << " degrees" << std::endl;
        }
    }
    if (verbosity >= 1) {
        std::cout << "    >> [rotation_index] total angle: " << total_angle
                  << std::endl;
    }
...
T gauss_bonnet_loop_solid_angle(const packed_float3* pts, uint iStart, uint N,
    ...
    for (uint i = 0; i < N; i++) {
        ...
        T corner_angle = atan2(dot(xp[b], cross(n_prev, n_next)),
                               Lp[b] * dot(n_prev, n_next));

        total_angle += corner_angle;

        if (verbosity >= 1) {
            std::cout << "  corner " << i << " has angle " << corner_angle
                      << std::endl;
        }
...
T gauss_bonnet_solid_angle(const packed_float3* pts, const packed_uint3* loops,
                           ...
                           const std::vector<uint>& polygonLoops,
                           const std::array<T, 3>& x, std::array<T, 3>* grad,
                           int grad_mode = 0, int verbosity = 0) {
    T omega = 0;
    for (uint iL : polygonLoops) {
        uint iStart = loops[iL].x - globalStart;
        uint N      = loops[iL].y;

        omega += gauss_bonnet_loop_solid_angle<T>(pts, iStart, N, x, grad,
                                                  grad_mode, verbosity);
    }
...
T polygon_solid_angle(const packed_float3* pts, const packed_uint3* loops,
                      ...
                      const std::array<T, 3>& x, int solid_angle_mode,
                      std::array<T, 3>* grad, int grad_mode = 0,
                      bool use_quick_triangulation = false, int verbosity = 0) {
    switch (solid_angle_mode) {
    case 0:
        return triangulated_solid_angle(pts, loops, globalStart, polygonLoops,
                                        x, grad, grad_mode,
                                        use_quick_triangulation);
    case 1:
        return prequantum_solid_angle(pts, loops, globalStart, polygonLoops, x,
                                      grad, grad_mode);
    case 2:
        return gauss_bonnet_solid_angle(pts, loops, globalStart, polygonLoops,
                                        x, grad, grad_mode, verbosity);
    default:
...
template <typename T>
T disk_solid_angle(const std::array<T, 3>& x, std::array<T, 3>* grad,
                   const std::array<T, 3>& disk_center,
                   const std::array<T, 3>& disk_normal, T disk_radius) {
    using T3 = std::array<T, 3>;
...
void classify_loops(const packed_float3* const pts,
                    const packed_uint3* const loops, uint n_loops,
                    uint global_start, std::vector<uint>* polygon_loops,
    ...
    for (uint iL = 0; iL < n_loops; iL++) {
        ...
        if (is_circular) {
            ...
            if (is_circular) {
                ...
                for (uint i = 1; i < N; i++) {
                    ...
                    if (std::abs(dot(normal, normal_i) -
                                 len(normal) * len(normal_i)) >= 0.01) {
                        is_circular = false;
                        break;
                    }
                }
        ...
        if (is_circular) {
            ...
            if (disk_centers) disk_centers->push_back(center);
            if (disk_normals) disk_normals->push_back(normal);
            if (disk_radii) disk_radii->push_back(radius);
        } else {
            // if loop was not determined to be circular, record it as a polygon
            if (polygon_loops) polygon_loops->push_back(iL);
        }
    }
}
...
ccl_device bool ray_nonplanar_polygon_intersect_T(
    ...
    if (params.clip_y) {
        if (ray_P[1] <= 0 && ray_D[1] <= 0) { // moving away from clipping plane
            return false;
                   ...
                   0) { // moving towards clipping plane from far side
            ray_tmin = fmax(ray_tmin, -ray_P[1] / ray_D[1]); // p + t * d = 0
        } else if (ray_D[1] <=
                   0) { // moving towards clipping plane from near side
    ...
    auto total_solid_angle = [&](const T3& x, T3& grad) -> T {
        ...
        for (uint iD = 0; iD < diskCenters.size(); iD++)
            omega += disk_solid_angle(x, &grad, diskCenters[iD],
                                      diskNormals[iD], diskRadii[iD]);
    ...
    while (t < ray_tmax) {
        ...
        if (r >= t_overstep) { // commit to step
            ...
            if (!params.fixed_step_count &&
                close_to_zero(val, lo_bound, hi_bound, (T)params.epsilon_loose,
                              grad, params.use_grad_termination)) {
                    ...
                    if (R < 0.25) {
                        ...
                        for (int i = 0; i < 5; i++) {
                            ...
                            bool close = close_to_zero(
                                val, lo_bound, hi_bound, epsilon, grad,
                                params.use_grad_termination);
            ...
            if (!params.fixed_step_count && params.use_extrapolation) {
                ...
                if (0. <= t_test && t_test <= 4. * (t + t_overstep - t_prev)) {
                    if (close_to_zero(e_val, lo_bound, hi_bound, epsilon,
                                      e_grad, params.use_grad_termination)) {
                        if (stats) stats->successful_extrapolations++;
                        return true;
                    }
                }
...
template <typename T>
ccl_device bool
newton_intersect_T(const solid_angle_intersection_params& params,
                   ccl_private float* isect_u, ccl_private float* isect_v,
                   ccl_private float* isect_t,
    ...
    for (int iN = 0; iN < 8; iN++) {
        ...
        if (close_to_zero(val, lo_bound, hi_bound, epsilon, grad_f,
                          params.use_grad_termination)) {
            *isect_t = t;
...
ccl_device float3 ray_nonplanar_polygon_normal_T(const float3 pf,
    ...
    classify_loops(pts, loops, n_loops, globalStart, &polygonLoops,
                   &diskCenters, &diskNormals, &diskRadii);
...
template <typename T>
ccl_device float3 ray_spherical_harmonic_normal_T(const float3 pf, uint m,
                                                  int l) {
    using T3 = std::array<T, 3>;
