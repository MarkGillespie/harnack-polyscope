...
template<typename T> interval<T> iMinMax(T x, T y, T z, T w)
{
  return {std::min(x, std::min(y, std::min(z, w))), std::max(x, std::max(y, std::max(z, w)))};
}

template<typename T> interval<T> iNeg(interval<T> x)
{  // -x
  return {-x.u, -x.l};
}

template<typename T> interval<T> iInv(interval<T> x)
{  // 1 / x
  if (x.l > 0.0 || x.u < 0.0) {
    return {(T)1.0 / x.u, (T)1.0 / x.l};
  }
  else if (x.l < 0.0 && x.u > 0.0) {
    return {-std::numeric_limits<T>::infinity(), std::numeric_limits<T>::infinity()};
  }
  else if (x.u == 0.0) {
    return {-std::numeric_limits<T>::infinity(), (T)1.0 / x.l};
  }
  else {
    return {(T)1.0 / x.u, std::numeric_limits<T>::infinity()};
  }
}

template<typename T> interval<T> iAdd(interval<T> x, interval<T> y)
{
  return {x.l + y.l, x.u + y.u};
}
template<typename T> interval<T> iSub(interval<T> x, interval<T> y)
{
  return {x.l - y.u, x.u - y.l};
}
template<typename T> interval<T> iMul(interval<T> x, interval<T> y)
{
  return iMinMax(x.l * y.l, x.l * y.u, x.u * y.l, x.u * y.u);
}
template<typename T> interval<T> iDiv(interval<T> x, interval<T> y)
{
  return iMul(x, iInv(y));
}

template<typename T> interval<T> iFloor(interval<T> x)
{
  return {std::floor(x.l), std::floor(x.u)};
}

template<typename T> interval<T> iMod(interval<T> x, interval<T> y)
{
  return iSub(x, iMul(y, iFloor(iDiv(x, y))));
}

template<typename T> interval<T> iSqrt(interval<T> x)
{
  if (x.l > 0.0)
    return {sqrt(x.l), sqrt(x.u)};
  if (x.u > 0.0)
    return {0.0, sqrt(x.u)};
  if (x.u == 0.0)
    return {0, 0};
...
template<typename T> interval<T> iAtan(interval<T> x)
{
  return {atan(x.l), atan(x.u)};
}

template<typename T> interval<T> iAtan2(interval<T> y, interval<T> x)
{
  if (x.u < 0.) {
    if (y.u < 0.)
      return {atan2(y.u, x.l), atan2(y.l, x.u)};
    if (y.l < 0.)
      return {-M_PI, M_PI};
  ...
  if (x.l < 0.) {
    if (y.u < 0.)
      return {atan2(y.u, x.l), atan2(y.u, x.u)};
    if (y.u == 0.) {
      if (y.l == 0.)
        return {0., M_PI};
      return {-M_PI, M_PI};
...
template<typename T> interval<T> iSin(interval<T> x)
{
  T u = M_PI * round(x.l / M_PI) + M_PI * 1.5;
  ...
  if (u < x.u) {
    T s = sin(u);
    return {min(rl, s), max(ru, s)};
  }

  return {rl, ru};
}

template<typename T> interval<T> iCos(interval<T> x)
{
  return iSin(x + interval<T>{M_PI / 2., M_PI / 2.});
}

#define MAKE_OVERLOADS(fn) \
  template<typename T> interval<T> fn(T x, const interval<T> &y) \
  { \
    return fn(interval<T>{x, x}, y); \
  } \
  template<typename T> interval<T> fn(const interval<T> &x, T y) \
  { \
...
template<typename T> struct i_vec3 {
  interval<T> x, y, z;
};

template<typename T> i_vec3<T> iAdd(const i_vec3<T> &a, const i_vec3<T> &b)
{
  return {a.x + b.x, a.y + b.y, a.z + b.z};
}
template<typename T> i_vec3<T> iSub(const i_vec3<T> &a, const i_vec3<T> &b)
{
  return {a.x - b.x, a.y - b.y, a.z - b.z};
}
template<typename T> i_vec3<T> iMul(const i_vec3<T> &a, const interval<T> &b)
{
  return {a.x * b, a.y * b, a.z * b};
}
template<typename T> i_vec3<T> iDiv(const i_vec3<T> &a, const interval<T> &b)
{
  return {a.x / b, a.y / b, a.z / b};
}
template<typename T> interval<T> iDot(const i_vec3<T> &a, const i_vec3<T> &b)
{
  return (a.x * b.x) + (a.y * b.y) + (a.z * b.z);
}
template<typename T> i_vec3<T> iCross(const i_vec3<T> &a, const i_vec3<T> &b)
{
  return {a.y * b.z - a.z * b.y,  //
          a.z * b.x - a.x * b.z,  //
          a.x * b.y - a.y * b.x};
}
template<typename T> interval<T> iLen(const i_vec3<T> &a)
{
  return iSqrt(iDot(a, a));
}

template<typename T> i_vec3<T> operator+(const i_vec3<T> &a, const i_vec3<T> &b)
{
  return iAdd(a, b);
}
...
template<typename T> i_vec3<T> operator*(const i_vec3<T> &a, const interval<T> &b)
{
  return iMul(a, b);
}
template<typename T> i_vec3<T> operator*(const interval<T> &a, const i_vec3<T> &b)
{
  return iMul(b, a);
}
template<typename T> i_vec3<T> operator/(const i_vec3<T> &a, const interval<T> &b)
...
template<typename T> struct i_cplx {
  interval<T> re, im;
...
template<typename T> interval<T> iArg(const i_cplx<T> &z)
{
